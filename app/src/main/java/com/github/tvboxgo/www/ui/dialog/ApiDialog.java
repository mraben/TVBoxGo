...
import com.github.tvboxgo.www.ui.adapter.ApiHistoryDialogAdapter;
...
public class ApiDialog extends BaseDialog {
    ...
    public ApiDialog(@NonNull @NotNull Context context) {
        ...
        findViewById(R.id.apinoraml).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ArrayList<String> normal = Config.getNormalUrl();
                Log.e("answer","列表："+new Gson().toJson(normal));
                if (normal.isEmpty())
                    return;
                String current = Hawk.get(HawkConfig.API_URL, "");
                int idx = -1;
                if (normal.contains(current))
                    idx = normal.indexOf(current);
                ApiNormalDialog dialog = new ApiNormalDialog(getContext());
                dialog.setTip("默认配置列表");
                dialog.setAdapter(new ApiNormalDialogAdapter.SelectDialogInterface() {
                    @Override
        ...
        findViewById(R.id.apiHistory).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ArrayList<String> history = Hawk.get(HawkConfig.API_HISTORY, new ArrayList<String>());
                Log.e("answer","列表："+new Gson().toJson(history));
                if (history.isEmpty())
                    return;
                String current = Hawk.get(HawkConfig.API_URL, "");
                int idx = 0;
